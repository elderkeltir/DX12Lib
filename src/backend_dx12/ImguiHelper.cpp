...
{
	HWND hwnd = *(HWND*)(gBackend->GetSwapChain()->GetWindowHndl().creation_data);
	m_device = device;
	m_frames_num = frames_num;

	m_rt->Initialize();
	std::vector<ResourceFormat> formats{ ResourceFormat::rf_r8g8b8a8_unorm };
	m_rt->CreateQuadTexture(gBackend->GetSwapChain()->GetWidth(), gBackend->GetSwapChain()->GetHeight(), formats, m_frames_num, 0, L"imgui_quad");
...
void ImguiHelper::Render(uint32_t frame_id)
{
	...
	if (std::shared_ptr<IGpuResource> rt = m_rt->GetRt(frame_id).lock()) {
		std::vector<IGpuResource*> rts{ rt.get()};
