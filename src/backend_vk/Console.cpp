...
AppConsole::AppConsole()
{
	...
	auto &command_names = ConsoleCommands::GetCommandNames();
...
void AppConsole::AddToLog(const std::string& line)
{
	if (Items.size() > 1024 * 32) {
		ClearLog();
	}
...
void AppConsole::ExecCommand(const char* command_line)
{
	...
	{
		ConsoleCommands::ExecuteCommand(command_line);
	}
