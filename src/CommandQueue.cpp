...
void CommandQueue::OnInit(ComPtr<ID3D12Device2> device, D3D12_COMMAND_LIST_TYPE type, uint32_t command_list_num, std::optional<std::wstring> dbg_name) {
...
void CommandQueue::ResourceBarrier(std::shared_ptr<GpuResource>& res, uint32_t to) {
...
void CommandQueue::ResourceBarrier(GpuResource& res, uint32_t to) {
    if (std::shared_ptr<HeapBuffer> buff = res.GetBuffer().lock()) {
        D3D12_RESOURCE_STATES calculated_from = (D3D12_RESOURCE_STATES)res.GetState();
        D3D12_RESOURCE_STATES to_native = (D3D12_RESOURCE_STATES)to;
        if (calculated_from != to_native) {
...
void CommandQueue::ResourceBarrier(std::vector<std::shared_ptr<GpuResource>>& res, uint32_t to) {
...
void CommandQueue::SetPSO(uint32_t id) {
    m_pso = id;
...
void CommandQueue::SetRootSign(uint32_t id, bool gfx) {
    m_root_sign = id;
